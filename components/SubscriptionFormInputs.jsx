...
export default function SubscriptionFormInputs({
  buttonText,
  fieldText,
  yogaClasses,
  selectionList,
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <FormControl
        variant='outlined'
        style={{ width: '65%', maxWidth: '200px' }}
      >
        <InputLabel>Select a class</InputLabel>
        <Select
          key={selectedYogaClass.id}
          value={selectedYogaClass.id}
          onChange={(e) => {
            const selectedClass = yogaClasses.find(
              (yogaClass) => yogaClass.id == e.target.value
            )
